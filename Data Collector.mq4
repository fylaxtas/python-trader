...
extern string Symbols="EURUSD,GBPUSD,USDJPY,USDCAD,EURJPY,AUDUSD,GBPJPY";
extern bool ListAll=False;
string Prefix="";
string Suffix="";
     ...
     //--- Split the string to substrings
      ...
      if (ListAll)
      {
         for(int i=0;i<SymbolsTotal(1);i++)
        {
         ...
         SymbolsArray[size]=SymbolName(i,1);
        }      
      }
      else
      {
         for (int symb=0; symb<ArraySize(tempSymbolsArray); symb++)
         {
            SymbolsArray[symb]=Prefix+tempSymbolsArray[symb]+Suffix;
         }      
      }
...
void CollectFunction (ENUM_TIMEFRAMES sTimeFrame=PERIOD_M1, string symbol="")
{
      ...
      file_handle =FileOpen("Data"+"/"+symbol+"_"+TFtoString(sTimeFrame)+".csv",FILE_CSV|FILE_READ|FILE_WRITE,',');
